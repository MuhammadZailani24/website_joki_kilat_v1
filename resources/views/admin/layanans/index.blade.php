...
<head>
    ...
    <title>Layanan & Harga - Admin Joki Kilat</title>
    ...
    <main class="flex-1 flex flex-col h-full relative overflow-y-auto overflow-x-hidden w-full">
        <header class="h-16 flex items-center justify-between px-4 lg:px-8 border-b border-zinc-800 bg-zinc-950/80 backdrop-blur-md sticky top-0 z-10 w-full">
            <div class="flex items-center gap-3">
                ...
                <h1 class="text-lg font-bold text-white truncate">Layanan & Harga</h1>
            </div>
        </header>
        ...
        <div class="p-4 lg:p-8 w-full">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-8">
                <div class="bg-zinc-900 border border-zinc-800 rounded-2xl p-4 md:p-6 shadow-xl h-fit">
                    <h3 class="text-lg font-bold text-white mb-4">➕ Tambah Layanan</h3>
                    <form action="{{ route('admin.layanans.store') }}" method="POST">
                        @csrf
                        <div class="space-y-4">
                            <div><label class="block text-[10px] md:text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">Nama Layanan</label><input type="text" name="nama_layanan" required placeholder="Contoh: Makalah" class="w-full bg-zinc-950 border border-zinc-700 text-white rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:border-blue-500 transition"></div>
                            <div>
                                <label class="block text-[10px] md:text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">Jenjang</label>
                                <select name="kategori" required class="w-full bg-zinc-950 border border-zinc-700 text-white rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:border-blue-500 transition">
                                    <option value="SD">SD</option><option value="SMP">SMP</option><option value="SMA/SMK">SMA / SMK</option><option value="Kuliah">Kuliah</option><option value="Aplikasi Web">Aplikasi Web</option><option value="Aplikasi Mobile">Aplikasi Mobile</option>
                                </select>
                            </div>
                            <div><label class="block text-[10px] md:text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">Harga (Rp)</label><input type="number" name="harga" required placeholder="50000" class="w-full bg-zinc-950 border border-zinc-700 text-white rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:border-blue-500 transition"></div>
                            <div><label class="block text-[10px] md:text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">Deskripsi</label><textarea name="deskripsi" rows="3" placeholder="Opsional..." class="w-full bg-zinc-950 border border-zinc-700 text-white rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:border-blue-500 transition"></textarea></div>
                            <button type="submit" class="w-full bg-blue-600 hover:bg-blue-500 text-white font-bold py-3 rounded-xl transition shadow-[0_0_15px_rgba(37,99,235,0.4)]">Simpan</button>
                        </div>
                    </form>
                </div>

                <div class="lg:col-span-2 bg-zinc-900 border border-zinc-800 rounded-2xl shadow-xl overflow-hidden w-full">
                    <div class="overflow-x-auto w-full">
                        <table class="w-full text-left border-collapse min-w-[600px]">
                            <thead>
                                <tr class="bg-zinc-950/50 text-gray-400 text-[10px] md:text-xs uppercase tracking-wider border-b border-zinc-800">
                                    <th class="py-4 px-4 font-bold">Layanan</th>
                            ...
                            <tbody class="text-sm text-gray-300">
                                @forelse($layanans as $item)
                                    <tr class="border-b border-zinc-800/50 hover:bg-zinc-800/30 transition">
                                        <td class="py-3 px-4"><p class="font-bold text-white">{{ $item->nama_layanan }}</p></td>
                                        <td class="py-3 px-4"><span class="bg-blue-500/10 text-blue-400 px-2 py-1 rounded text-[10px] md:text-xs font-bold border border-blue-500/20 whitespace-nowrap">{{ $item->kategori }}</span></td>
                                        <td class="py-3 px-4 font-bold text-green-400 whitespace-nowrap">Rp {{ number_format($item->harga, 0, ',', '.') }}</td>
                                        <td class="py-3 px-4">
                                            <div class="flex justify-center gap-2">
                                                <form action="{{ route('admin.layanans.destroy', $item->id) }}" method="POST" onsubmit="return confirm('Hapus layanan?')">
                                                    @csrf @method('DELETE')
                                ...
                                @empty
                                    <tr><td colspan="4" class="py-8 text-center text-gray-500">Belum ada layanan.</td></tr>
                                @endforelse
                            </tbody>
                        </table>
                    </div>
